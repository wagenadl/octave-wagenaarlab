function y = mog_limitedem(X,par,epsi,lambda)
% MOG_LIMITEDEM implements the EM algorithm for mixture of Gaussians.
% Input: X: DxN data 
...
% Coding:    Daniel Wagenaar, April-May 2000, May 2004
% This is a special version of MOG_FULLEM, which does not modify the 
% parameters mu,sig,p of the last cluster, no matter what.
...
for iter = 1:max_iter
  ...
  newp = mean(R,1);
  p(1:K-1) = newp(1:K-1) * (1-p(K))/sum(newp(1:K-1));
