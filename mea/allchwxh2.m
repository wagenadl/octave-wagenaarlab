function allchwxh2(spks,binsw,binsh,ax)
% ALLCHWXH2(spks,binsw,binsh,ax) plots 60 scatter plots of the 
...
% This vsn uses a logarithmic scale (log10 colorbar plotted)
...
%    wnow = wnow + wrnd;
  ...
  %  figure(1);
    subplot(8,8,hw2cr(i));
    pcolor(repmat(ww,[1 size(ct,2)]),repmat(hh,[size(ct,1) 1]),log10(ct));
    colormap(cmap);
    shading flat;
    colorbar;
    if nargin>=4
