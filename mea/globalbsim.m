...
% bursts = GLOBALBSIM(times, channels, mthr, thr1, thr0, wid, div) 
...
%   (This is CC).
% - Find peaks in this histo using SIMPLEPEAKS.
...
pk = simplepeaks(cc,thr1,.25,thr0);
peaktime = tt(pk.pos);

for n=1:length(pk.pos)
  x1 = pk.onset(n);
  x2 = pk.offset(n);
  onset=[onset tt(x1)];
  offset=[offset tt(x2)];
