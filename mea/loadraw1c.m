...
%    y=LOADRAW1C(fn, hw, range) reads the raw MEA datafile fn and converts the
...
%    y = LOADRAW1C(fn, hw, range, lim) reads only the first LIM scans.
%    y = LOADRAW1C(fn, hw, range, lim, skip) skips the first SKIP scans and then
%    reads LIM scans.
%    LOADRAW1C will also load followups (files named FN-1, FN-2, etc. ).
...
while 1
  ...
  z = fread(fh,[64 min(lim-length(y), 64*1024)],'int16');
  if isempty(z)
    break;
  else
    y = [y z(hw+1,:)];
