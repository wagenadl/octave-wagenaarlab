...
for k=1:999
  ...
  if exist(fn)
    ...
    if length(x2.t_o) == size(mn,1)
      % Normal situation
      cc_mean{k} = mn(1:2:end,:);
      cc_sem{k} = st(1:2:end,:)./sqrt(nn(1:2:end,:));
      cc_tms{k} = x2.t_o(1:2:end);
      ox_mean{k} = mn(2:2:end,:);
      ox_sem{k} = st(2:2:end,:)./sqrt(nn(2:2:end,:));
      ox_tms{k} = x2.t_o(2:2:end);
    else
      % Single snapshot
      cc_mean{k} = mn(3:4,:);
      cc_sem{k} = st(3:4,:)./sqrt(nn(3:4,:));
      cc_tms{k} = [.2 .25]'; % Not so great, but oh well...
      ox_mean{k} = mn(1:2,:);
      ox_sem{k} = st(1:2,:)./sqrt(nn(1:2,:));
      ox_tms{k} = [0 .05]'; 
    end
    idx = [idx; k];
  end
...
save(ofn,'elps','opt_npix','idx',...
    ...
    'cc_mean','cc_sem','cc_tms','ox_mean','ox_sem','ox_tms');
