...
static coord *oldpile = 0;
static coord *newpile = 0;
static int Nalloc = 0;

static void op_flood(unsigned char const *img, int X, int Y,
		     unsigned int *obj, int x, int y, int k) {
  int Nold = 0;
  int Nnew = 0;
  oldpile[0].x = x;
  oldpile[0].y = y;
...
static void op_objectify(unsigned char const *img, int X, int Y,
			 unsigned int *obj) {
  int x, y;
  int k=0;
  ...
  Nalloc = 1024;
  oldpile = (coord*)mxMalloc(Nalloc*sizeof(coord));
  newpile = (coord*)mxMalloc(Nalloc*sizeof(coord));

  for (y=0; y<Y; y++) {
    unsigned char const *row = img + y*X;
    unsigned int const *orow = obj + y*X;
    for (x=0; x<X; x++) {
      if (row[x] && !orow[x])
	op_flood(img, X, Y, obj, x, y, ++k);
    }
  }
