function y = mosg_partialem(X,par,epsi,lambda,idx)
% MOSG_PARTIAL implements the partial update alg for Mosg from Ueda et al
% Input: X: DxN data 
...
for iter = 1:max_iter
  ...
  for k = idx
    ...
    sig{k} = (Rsdxdx + D*lambda)/(D*(sumR+lambda));
    p(k) = mean(R(:,k));
  end
