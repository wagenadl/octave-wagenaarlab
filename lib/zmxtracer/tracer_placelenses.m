...
for k=1:length(trc.lenses)
  if isnumeric(trc.lenses{k})
    trc.x_surf = [trc.x_surf; trc.xlens(k)];
    trc.r_surf = [trc.r_surf; trc.lenses{k}]; % slightly abusive
    trc.dn_surf = [trc.dn_surf; nan]; % use nan as a marker for an ideal lens
    trc.xlens1(k) = trc.xlens(k);
  else
