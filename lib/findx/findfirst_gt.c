/* findfirst_gt.c - heavily based on xtimesy.c in the matlab examples */

#include "mex.h"

int findfirst_gt(double const *src, double tst, int n) {
  int i;
  for (i=0; i<n; i++) 
    if (src[i]>tst)
      return i+1;
 ...
 */
  ...
  *idxout = findfirst_gt(haystack,needle,mrows*ncols);
