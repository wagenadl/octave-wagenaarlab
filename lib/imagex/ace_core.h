...
#ifndef CONVNORM_CORE_H

#define CONVNORM_CORE_H
...
public:
  ...
  int width() const;
  int height() const;
  DoubleImage &operator=(QImage const &);
  ...
  DoubleImage ace(double sx, int rx, double sy, int ry,
                 bool showProgress=false) const;
  void apply(double (*foo)(double));
  void noisify();
  double mean() const;
  static DoubleImage gaussian(double sigmax, int rx, double sigmay=-1, int ry=-1);
private:
