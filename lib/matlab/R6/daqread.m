...
% Backported to Matlab V6 by Daniel Wagenaar.

v=version('-release');
if v>14
  fprintf(1,'Warning: you should probably be using the builtin version of DAQREAD\n');
end
...
if ~ischar(filename)
   error('Invalid FILENAME specified to DAQREAD.');
end
...
if fid < 3
   error(sprintf('Unable to open specified FILENAME: ''%s''.', filename));
end
...
if ~( strcmp(FileKey,['MATLAB Data Acquisition File.' 0 25 0]))
   fclose(fid);
   error('FILENAME is not a valid Data Acquisition Toolbox file.');
end
...
if flag
   ...
   if isempty(num_of_blocks)
      try
         ...
         fclose(fid);
         error('Data acquisition file is corrupted and cannot be read.');
      end
   end
...
try
	...
	warning('Data acquisition file is corrupted.  May not be able to read the requested samples.');
	warning(warnstate);
end;   
...
case 2   % DAQINFO = daqread(FILENAME, 'info');
   ...
   if ~strcmp(lower(varargin{1}), 'info')
      fclose(fid);
      error('Invalid second argument specified.');
   end
...
if nargout >= 0 & all(sampleTimes ~= -1)
   ...
   if ~isempty(timerange)
      ...
      if ~strcmp(lower(clockSource), 'internal')
         warning(['The requested samples may be incorrect if the SampleRate property does not \n',...
   ...
   if ~isempty(triggers)
      if ~isinf(samplesPerTrigger)
         ...
         fclose(fid);
         error('Error reading the file.  Try specifying the SAMPLES range.');
      end
   end
   ...
   if ~isempty(samples) & ~isempty(samplesacquired)
      ...
      if samples(2) > max_available
         samples(2) = max_available;
            warning('More samples than are available has been requested.\nOnly %s samples are available.', num2str(max_available));
      end
   end
   ...
   try
      [data, nanLoc] = localReadData(fid,chart,num_chans,samples,channels,objinfo);
       ...
       warnstate = warning('on');
       warning('The data file is corrupted.  All the requested samples may not have been read.');
       warning(warnstate);
   end
...
if strncmp(lower(dataformat), 'double', length(dataformat))
   try
      if ~isempty(data)
       ...
       warnstate = warning('on');
       warning('An error occurred while converting data to double format.\nTry specifying ''native'' DATAFORMAT.');
       warning(warnstate);
      fclose(fid);
      return;
   end
