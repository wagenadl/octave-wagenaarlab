...
raw_t LocalFit::ZERO=NAN;

//--------------------------------------------------------------------
// inline functions
//
inline void LocalFit::update_X012() {
  float_t y_new = source[t_stream+tau];
  float_t y_old = source[t_stream-tau-1];
  X0 += y_new - y_old;
...
//
LocalFit::LocalFit(raw_t *source0, raw_t *dest0, timeref_t t_start,
