/* findfirst.c - heavily based on xtimesy.c in the matlab examples */

#include "mex.h"

int findfirst_ne(double const *src, double tst, int n) {
  int i;
  for (i=0; i<n; i++) 
    if (src[i]!=tst)
      return i+1;
 ...
 */
  ...
  *idxout = findfirst_ne(haystack,needle,mrows*ncols);
