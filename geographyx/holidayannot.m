...
function [hol,ann] = holidays (sd, ed)
  ...
  hol = [hol; tmphol(:)];
  for k=1:length(tmphol)
    ann{end+1} = 'M L King Day';
  end
  ## Washington's Birthday, the third Monday in February
  tmphol = nweekdate (3, 2, yrs, 2);
  hol = [hol; tmphol(:)];
  for k=1:length(tmphol)
    ann{end+1} = 'Presidents Day';
  end
  ## Good Friday
  tmphol = easter (yrs) - 2;
  hol = [hol; tmphol(:)];
  for k=1:length(tmphol)
    ann{end+1} = 'Good Friday';
  end
  ## Memorial Day, the last Monday in May
  tmphol = lweekdate (2, yrs, 5);
  hol = [hol; tmphol(:)];
  for k=1:length(tmphol)
    ann{end+1} = 'Memorial Day';
  end
  ## Independence Day, July 4
  tmphol = datenum (yrs, 7, 4);
  hol = [hol; tmphol(:)];
  for k=1:length(tmphol)
    ann{end+1} = 'Independence Day';
  end
  ...
  if nargout<2
	...
	s = [s sprintf(',\n')];
      ...
      s = [s '"' datestr(hol(k),'yyyymmdd') '": "' ann{k} '"' ];
    end
    s = [ s '}'];
    hol = s;
  end
